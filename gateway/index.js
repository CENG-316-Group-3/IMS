...
app.post('/getApplication', async (req, res) => {
    ...
    const msg = JSON.stringify(content);
    const correlationId = generateUuid();
    const correlationId2 = generateUuid();
    emitMessageCorrelationId('/getStudent', msg, correlationId);
    emitMessageCorrelationId('/company/getApplicationsById', msg, correlationId2);
    const response = await waitForResponse(correlationId);
    const response2 = await waitForResponse(correlationId2);
    value = JSON.parse(response);
    value2 = JSON.parse(response2);
    if (value.stat === 200){
        res.status(200).json({value2,value});
    }else{
...
app.post('/applicationLetter', async (req, res) => {
    const content = req.body;
    const msg = JSON.stringify(content);
    const correlationId = generateUuid();
...
app.post('/student/status', async (req, res) => {
    const content = req.body;
    const msg = JSON.stringify(content);
    const correlationId = generateUuid();
...
app.post('/company/getApplicationsById', async (req, res) => {
    const content = req.body;
    const msg = JSON.stringify(content);
    const correlationId = generateUuid();
...
app.post('/company/getApplications', async (req, res) => {
    const content = req.body;
    const msg = JSON.stringify(content);
    const correlationId = generateUuid();
...
app.post('/ims/student/coordinator-announcement', async(req, res) => { 
    try{
        const content = req.body;
        const msg = JSON.stringify(content);
        const correlationId = generateUuid();
...
//notification
app.post('/ims/notifications', async(req, res) => { 
    try{
        const content = req.body;
        const msg = JSON.stringify(content);
...
app.post('/ims/notification', async(req, res) => { 
    try{
        const content = req.body;
        const msg = JSON.stringify(content);
