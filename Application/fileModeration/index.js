...
amqp.connect(rabbitMQUrl, function(error0, connection) {
    ...
    connection.createChannel(function(error1, ch) {
        ...
        const routingKeys = ['application letter create', 'application form create', 'application form accept'
            ,'/applicationLetter', '/applicationForm', 'cancel application', 'cancel application form','create SSI document', '/getSSI',"/getStudent"
        ];
        ...
        channel.assertQueue(queueName, { exclusive: true }, function(error2, q) {
            ...
            channel.consume(q.queue, function(message) {
                ...
                else if(message.fields.routingKey === '/getStudent'){
                    processGetStudent(message)}
            }, { noAck: true });
        });
    });
});
...
async function processGetStudent(message){
    try {
        ...
        emitMessageCorrelationId('success',JSON.stringify( addStatus(student, 200)), message.properties.correlationId);
    } catch (error) {
